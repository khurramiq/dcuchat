...
const Tab1 = ({ setLastQuiz }) => {
  ...
  useEffect(() => {
    if (recentChapterId !== "" && recentLessonId !== "") {
      // My Learning page
      dispatch(getCourseById(_User.profile?.recentCourse));
      dispatch(getLessonsByChapterId(recentChapterId));
      dispatch(getChapterById(recentChapterId));
      dispatch(getLessonById(recentLessonId));
      dispatch(getQuizzesByLessonId(recentLessonId));
      dispatch(getAllChaptersByCourseId(_User.profile?.recentCourse));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [recentChapterId, recentLessonId]);
